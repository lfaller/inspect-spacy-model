...
def list_available_models():
    ...
    try:
        ...
        for package in installed_packages:
            if any(lang in package for lang in ['en_', 'de_', 'fr_', 'es_', 'pt_', 'it_', 'nl_', 'zh_']):
        ...
        if spacy_models:
            ...
            for model in sorted(spacy_models):
                try:
                    ...
                    print(f"   ❌ {model} (installation issue)")
        else:
...
def inspect_spacy_model(model_name: str = "en_core_web_sm", verbose: bool = False):
    ...
    if nlp.vocab.vectors.shape[0] > 0:
        ...
        print(f"   Vectors: No word vectors available")
    ...
    if doc.ents:
        ...
        print(f"   Entities: No entities detected")
    
    if verbose:
        print(f"   POS Tags: {[(token.text, token.pos_) for token in doc]}")
        print(f"   Lemmas: {[(token.text, token.lemma_) for token in doc if token.text != token.lemma_]}")
...
Examples:
  ...
  %(prog)s en_core_web_lg --verbose  # Detailed inspection
    ...
    parser.add_argument(
        '--list', '-l',
        action='store_true',
        help='List all installed spaCy models'
    )
    ...
    parser.add_argument(
        '--version',
        action='version',
        version='spaCy Model Inspector 1.0.0'
    )
